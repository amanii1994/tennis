...
import { View, Text,TouchableHighlight, ScrollView,StyleSheet,AsyncStorage, Platform, Image, TouchableOpacity, Alert, Keyboard, TouchableWithoutFeedback } from 'react-native';
...
export default class tinygroupB extends Component {
    ...
    async componentDidMount() {
        ...
        if (this._isMounted) {
            this.setState({saveData: this.props.navigation.getParam('itemData')});
        }
    ...
    async onCardNonceRequestSuccess(cardDetails) {
        if (this.chargeServerHostIsSet()) {
            try {
                ...
                let res = restapi.post(Constants.CHARGE_SERVER_URL, { card_nounce: cardDetails.nonce, amount: this.state.saveData.price * this.state.saveData.quantity, activity_name: this.state.saveData.activity_name });
                res.then(res => {
        ...
        return (
            <View style={{ backgroundColor: '#fff', width: '100%', height: '100%', flex: 1 }}>
                ...
                <View style={styles.container}>
                    <TouchableOpacity style={{ alignSelf: 'center', marginLeft: wp('3%'), }} onPress={() => navigate('tinygroupA')}><Linericon name="left-arrow-1" size={wp('7.5%')} color='#000000' /></TouchableOpacity>
                    ...
                    <View style={styles.modalContent}>
                        ...
                        <ScrollView>
                            ...
                            <View style={stylesBody.row}>
                                ...
                                <View style={stylesBody.descriptionColumn}>
                                    <Text style={stylesBody.bodyText}>$ {this.state.saveData.price * this.state.saveData.quantity}</Text>
                                </View>
                            </View>
                            <View style={stylesBody.horizontalLine} />
