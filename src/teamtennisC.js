...
import { View, Text, StyleSheet, Platform, Image, Alert,TouchableOpacity, Keyboard,ScrollView, TouchableHighlight,AsyncStorage } from 'react-native';
...
export default class teamtennisC extends Component {
    constructor(props) {
        super(props);
        this.state = {
            saveData : {},
    ...
    async onCardNonceRequestSuccess(cardDetails) {
        if (this.chargeServerHostIsSet()) {
            try {
                ...
                let res = restapi.post(Constants.CHARGE_SERVER_URL, { card_nounce: cardDetails.nonce, amount: this.state.saveData.price, activity_name: this.state.saveData.activity_name });
                res.then(res => {
        ...
        return (
                <View style={{ backgroundColor: '#fff', width: '100%', height: '100%', flex: 1 }}>
                    ...
                    <View style={styles.container}>
                        <TouchableOpacity style={{ alignSelf: 'center', marginLeft: wp('3%'), }} onPress={() => navigate('summerB')}><Linericon name="left-arrow-1" size={wp('7.5%')} color='#000000' /></TouchableOpacity>
                    ...
                    <View style={[{ marginLeft: wp('4%'), marginRight: wp('3%'), marginTop:wp('7%'),borderWidth:wp('0.4%'),borderColor:'#D5D5D5',padding:wp('5%') }]}>
                        <Text style={styles.textP}>Thank You for Reserving your spot for Tiny Tennis Team.</Text>
                    </View>
                    ...
                    <View style={{ flexDirection: 'row', alignSelf: 'center', marginTop:wp('3%') }}>
                        <View style={[styles.containerC, { padding: wp('1%') }]}>
                            ...
                            <View style={{ width: wp('95%'), justifyContent: 'space-around' }}>
                                <Text style={[styles.textcontainerC, { alignItems: 'flex-start',marginLeft: wp('5%') }]}> 9AM - 3PM</Text>
                            </View>
                        </View>
                    </View>
                    {/* <View style={{ flexDirection: 'row', alignSelf: 'center', marginTop:wp('3%') }}>
