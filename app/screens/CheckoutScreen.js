...
class CheckoutScreen extends Component {
  ...
  async componentWillMount() {
    AsyncStorage.getItem("itemReader").then((info) => {
      if (info) {
          this.setState({itemReader:JSON.parse(info)});
      }});
    try {
  ...
  async onCheckout() {
    ...
    const checkoutParams = {
      amountMoney: {
        amount: this.state.itemReader.total_price * 100,
      ...
      note: this.state.itemReader.activity_name,
      tipSettings: {
    ...
    try {
      ...
      this.state.itemReader.t_id = JSON.stringify(checkoutResult);
      let p = Rest.saveAppoint(pData);
    ...
    return (
      <View style={defaultStyles.pageContainer}>
        ...
        <View style={defaultStyles.buttonContainer}>
          <CustomButton
            title={"Charge $"+this.state.itemReader.total_price}
